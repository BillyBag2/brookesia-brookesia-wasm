...
Both launchers use its density of `1.0` and its font scale without a WASM
override. Hardware properties such as resolution and pixel format remain sourced
from the native BSP or the WASM hardware description. ESP-IDF-only preparation
is isolated under the submodule board's `native/` directory.
